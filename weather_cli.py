...
def output_weather(data, format_type):
    ...
    if format_type == 'json':
        ...
        print(f"ubicacion,temperatura,descripcion\n"
              f"{weather_info['ubicacion']},{weather_info['temperatura']},{weather_info['descripcion']}")
    else:
        print(f"Clima en {weather_info['ubicacion']}:\n"
...
def main():
    ...
    parser.add_argument('-l', '--location', type=str, required=True, help="Nombre de la ciudad o región para obtener el clima.")
    parser.add_argument('-f', '--format', type=str, choices=['json', 'csv', 'text'], default='text',
